    ...
    const handleMouseMove = e => {
      ...
      if (enableMagnetism) {
        magnetismAnimationRef.current = gsap.to(element, {
          x: (x - centerX) * 0.04, y: (y - centerY) * 0.04,
...
const GlobalSpotlight = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) => {
  ...
  useEffect(() => {
    ...
    spotlight.style.cssText = `position:fixed;width:600px;height:600px;border-radius:50%;pointer-events:none;background:radial-gradient(circle,rgba(${glowColor},0.12) 0%,rgba(${glowColor},0.06) 20%,rgba(${glowColor},0.03) 35%,transparent 65%);z-index:200;opacity:0;transform:translate(-50%,-50%);mix-blend-mode:screen;`;
    ...
    const handleMouseMove = e => {
      ...
      cards.forEach(card => {
        ...
        const distance = Math.max(0, Math.hypot(e.clientX - (cardRect.left + cardRect.width/2), e.clientY - (cardRect.top + cardRect.height/2)) - Math.max(cardRect.width, cardRect.height)/2);
        ...
        {cards.map((card, index) => (
          ...
          >
            ...
            <div className="magic-bento-card__content">
              {card.value !== undefined && <div className="magic-bento-card__value">{card.value}</div>}
